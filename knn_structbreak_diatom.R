...
#----------------------------------- wczytanie danych: DiatomSizeReduction -----------------------------------------------#
coffee_train <- read.table('/Users/Ula/Desktop/wtum/DiatomSizeReduction_TRAIN.txt')

coffee_test <- read.table('/Users/Ula/Desktop/wtum/DiatomSizeReduction_TEST.txt')

macierz_przelomow_coffee <- read.csv('/Users/Ula/Desktop/wtum/macierz_przelomow_diatom.csv')
...
# ----------------------------- METODA 2 ---------------------------------- #
macierz_przelomow_coffee # tu widac, ze niektore segmenty sa za krotkie

apply(macierz_przelomow_coffee, 1, function(x){
  
  x = x[!is.na(x)]
  
  DrawSeries2(t(coffee_train_val), breaks=x)
...
m2_acc_k1 = sum(labels1 == coffee_test_labels)/306

acc_poszczegolnych_k1=apply(macierz_klasyfikatorów1==coffee_test_labels, 2, sum)/306 # 0.9067 najlepiej
...
m2_acc_k3= sum(labels3 == coffee_test_labels)/306


acc_poszczegolnych_k3=apply(macierz_klasyfikatorów3==coffee_test_labels, 2, sum)/306# najlepszy 0.88
...
m2_acc_k5 = sum(labels5 == coffee_test_labels)/306

acc_poszczegolnych_k5=apply(macierz_klasyfikatorów5==coffee_test_labels, 2, sum)/306 # najlepszy 0.87
...
m2_acc_k7 = sum(labels7 == coffee_test_labels)/306

acc_poszczegolnych_k7=apply(macierz_klasyfikatorów7==coffee_test_labels, 2, sum)/306
...
m2_acc_k9 = sum(labels9 == coffee_test_labels)/306

acc_poszczegolnych_k9=apply(macierz_klasyfikatorów9==coffee_test_labels, 2, sum)/306
...
write.csv(wyniki_poszczegolnych, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_diatom.csv')
...
#14, 32, 40
komitet_najlepszych = getLabels(cbind(macierz_klasyfikatorów1[, c(2, 4:10, 14, 16)], macierz_klasyfikatorów3[, c(2, 3, 5:10,12, 16)]))
m2_acc_komitet_najlepszych = sum(komitet_najlepszych==coffee_test_labels)/304 #0.94

table(komitet_najlepszych,coffee_test_labels)

# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(labels1, labels3, labels5))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/306
...
m2_acc_k1s = sum(labels1s == coffee_test_labels)/306

acc_poszczegolnych_k1s=apply(macierz_klasyfikatorów1s==coffee_test_labels, 2, sum)/306 # 0.96
...
m2_acc_k3s= sum(labels3s == coffee_test_labels)/306

acc_poszczegolnych_k3s=apply(macierz_klasyfikatorów3s ==coffee_test_labels, 2, sum)/306
...
m2_acc_k5s = sum(labels5s == coffee_test_labels)/306

acc_poszczegolnych_k5s=apply(macierz_klasyfikatorów5s==coffee_test_labels, 2, sum)/306
...
m2_acc_k7s = sum(labels7s == coffee_test_labels)/306

acc_poszczegolnych_k7s=apply(macierz_klasyfikatorów7s==coffee_test_labels, 2, sum)/306
...
m2_acc_k9s = sum(labels9s== coffee_test_labels)/306

acc_poszczegolnych_k9s=apply(macierz_klasyfikatorów9s==coffee_test_labels, 2, sum)/306
...
write.csv(wyniki_poszczegolnych_s, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_diatom_scalone.csv')
...
komitet_najlepszych = getLabels(cbind(macierz_klasyfikatorów1s[, c(4, 5, 10, 11)]))

m2_acc_komitet_najlepszych_s = sum(komitet_najlepszych==coffee_test_labels)/306
