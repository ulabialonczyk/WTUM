...
#----------------------------------- wczytanie danych -----------------------------------------------#
coffee_train <- read.table('/Users/Ula/Desktop/wtum/Coffee_TRAIN.txt')

coffee_test <- read.table('/Users/Ula/Desktop/wtum/Coffee_TEST.txt')

przelomy_coffee <- read.table('/Users/Ula/Desktop/wtum/przelomy_coffee.csv') # jako factory sie zapisaly
...
# --------------------------------- szukanie etykiet ---------------------------#
przelomy_coffee = c(1,28,52,79,85,107,132, 157, 182, 206, 227, 247, 259)
...
predykcje_1kv = getLabels(knn_coffee[, -c(3,8,9,11)])
...
predykcje_3kv = getLabels(knn_coffee[, -c(3,8,9,11)])
...
predykcje_5kv = getLabels(knn_coffee[, -c(3,8,9,11)])
...
predykcje_7kv = getLabels(knn_coffee[, -c(3,8,9,11)])
...
predykcje_9kv = getLabels(knn_coffee[, -c(3,8,9,11)])
...
# odcinki 3, 8, 9, 11 maja najwieksza wariancje - tych odcinkow nie bierzemy pod uwagê przy knn
...
m2_acc_k1 = sum(etykietyk1 == coffee_test_labels)/28
...
sum(komitet_najlepszych==coffee_test_labels)/28 # obs 15 zle zaklasyfikowana

acc_poszczegolnych_k1=apply(macierz_etykiet==coffee_test_labels, 2, sum)/28 # 0.96
...
m2_acc_k3= sum(etykietyk3 == coffee_test_labels)/28

macierz_etykiet

acc_poszczegolnych_k3=apply(macierz_etykiet==coffee_test_labels, 2, sum)/28
...
m2_acc_k5 = sum(etykietyk5 == coffee_test_labels)/28

acc_poszczegolnych_k5=apply(macierz_etykiet==coffee_test_labels, 2, sum)/28
...
m2_acc_k7 = sum(etykietyk7 == coffee_test_labels)/28

acc_poszczegolnych_k7=apply(macierz_etykiet==coffee_test_labels, 2, sum)/28
...
m2_acc_k9 = sum(etykietyk9== coffee_test_labels)/28

acc_poszczegolnych_k9=apply(macierz_etykiet==coffee_test_labels, 2, sum)/28
...
# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(etykietyk1, etykietyk3, etykietyk5))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/28
