...
#----------------------------------- wczytanie danych: toe1 -----------------------------------------------#
coffee_train <- read.table('/Users/Ula/Desktop/wtum/ToeSegmentation1_TRAIN.txt')

coffee_test <- read.table('/Users/Ula/Desktop/wtum/ToeSegmentation1_TEST.txt')

macierz_przelomow_coffee <- read.csv('/Users/Ula/Desktop/wtum/macierz_przelomow_toe1.csv')
...
coffee_test_labels = coffee_test[, 1] + 1
coffe_labels_factor = as.factor(as.character(coffee_test[, 1]))

scalona_macierz_przelomow = apply(macierz_przelomow_coffee, 1, function(x){
...
# ---------------------------------- METODA 1 ---------------------------------------- #
przelomy = read.csv('/Users/Ula/Desktop/wtum/przelomy_toe1.csv')
...
przelomy2 = c(1, 18, 56, 94,126, 168, 199, 223, 247)
...
komitet2=cbind(predykcje1k2, predykcje3k2, predykcje9k2)
...
m2_acc_k1 = sum(labels1 == coffee_test_labels)/228

acc_poszczegolnych_k1=apply(macierz_klasyfikatorów1==coffee_test_labels, 2, sum)/228 
...
m2_acc_k3= sum(labels3 == coffee_test_labels)/228


acc_poszczegolnych_k3=apply(macierz_klasyfikatorów3==coffee_test_labels, 2, sum)/228
...
m2_acc_k5 = sum(labels5 == coffee_test_labels)/228

acc_poszczegolnych_k5=apply(macierz_klasyfikatorów5==coffee_test_labels, 2, sum)/228 # najlepszy 0.87
...
m2_acc_k7 = sum(labels7 == coffee_test_labels)/228

acc_poszczegolnych_k7=apply(macierz_klasyfikatorów7==coffee_test_labels, 2, sum)/228
...
m2_acc_k9 = sum(labels9 == coffee_test_labels)/228

acc_poszczegolnych_k9=apply(macierz_klasyfikatorów9==coffee_test_labels, 2, sum)/228
...
write.csv(wyniki_poszczegolnych, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_toe1.csv')
...
#14, 32, 40
komitet_najlepszych = getLabels(cbind(macierz_klasyfikatorów1[, c(7, 11, 31)],macierz_klasyfikatorów3[, 12], macierz_klasyfikatorów5[, 23]))
m2_acc_komitet_najlepszych = sum(komitet_najlepszych==coffee_test_labels)/228#0.94

table(komitet_najlepszych,coffee_test_labels)

# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(labels1, labels3, labels5))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/228
...
m2_acc_k1s = sum(labels1s == coffee_test_labels)/228

acc_poszczegolnych_k1s=apply(macierz_klasyfikatorów1s==coffee_test_labels, 2, sum)/228# 0.96
...
m2_acc_k3s= sum(labels3s == coffee_test_labels)/228

acc_poszczegolnych_k3s=apply(macierz_klasyfikatorów3s ==coffee_test_labels, 2, sum)/228
...
m2_acc_k5s = sum(labels5s == coffee_test_labels)/228

acc_poszczegolnych_k5s=apply(macierz_klasyfikatorów5s==coffee_test_labels, 2, sum)/228
...
m2_acc_k7s = sum(labels7s == coffee_test_labels)/228

acc_poszczegolnych_k7s=apply(macierz_klasyfikatorów7s==coffee_test_labels, 2, sum)/228
...
m2_acc_k9s = sum(labels9s== coffee_test_labels)/228

acc_poszczegolnych_k9s=apply(macierz_klasyfikatorów9s==coffee_test_labels, 2, sum)/228
...
write.csv(wyniki_poszczegolnych_s, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_toe1_scalone.csv')
...
komitet_najlepszych = getLabels(cbind(macierz_klasyfikatorów1s[, c(22, 33)], macierz_klasyfikatorów3s[, c(1,22)]))

m2_acc_komitet_najlepszych_s = sum(komitet_najlepszych==coffee_test_labels)/228

table(komitet_najlepszych, coffee_test_labels)
# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(labels3s, labels1s, labels7s))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/228
