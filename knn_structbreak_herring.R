...
coffee_train <- read.table('/Users/Ula/Desktop/wtum/Herring_TRAIN.txt')


coffee_test <- read.table('/Users/Ula/Desktop/wtum/Herring_TEST.txt')

macierz_przelomow_coffee <- read.csv('/Users/Ula/Desktop/wtum/macierz_przelomow_herring.csv')

rows2=apply(macierz_przelomow_coffee, 1, function(x){
  print(all(is.na(x)))
})
idx2 = which(rows2==TRUE) # wiersze macierzy z samymi NA

macierz_przelomow_coffee= macierz_przelomow_coffee[-idx2, ]
...
komitetz = cbind(knn_zwykle_coffee7, knn_zwykle_coffee5, knn_zwykle_coffee9)
...
m2_acc_k1 = sum(labels1 == coffee_test_labels)/64

acc_poszczegolnych_k1=apply(macierz_klasyfikatorów1==coffee_test_labels, 2, sum)/64 # 0.9067 najlepiej
...
m2_acc_k3= sum(labels3 == coffee_test_labels)/64


acc_poszczegolnych_k3=apply(macierz_klasyfikatorów3==coffee_test_labels, 2, sum)/64 # najlepszy 0.88
...
m2_acc_k5 = sum(labels5 == coffee_test_labels)/64

acc_poszczegolnych_k5=apply(macierz_klasyfikatorów5==coffee_test_labels, 2, sum)/64 # najlepszy 0.87
...
m2_acc_k7 = sum(labels7 == coffee_test_labels)/64

acc_poszczegolnych_k7=apply(macierz_klasyfikatorów7==coffee_test_labels, 2, sum)/64
...
m2_acc_k9 = sum(labels9 == coffee_test_labels)/64

acc_poszczegolnych_k9=apply(macierz_klasyfikatorów9==coffee_test_labels, 2, sum)/64
...
write.csv(wyniki_poszczegolnych, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_herring.csv')
...
komitet_najlepszych = getLabels(cbind(macierz_klasyfikatorów1[, 4], macierz_klasyfikatorów3[, c('42')]))
m2_acc_komitet_najlepszych = sum(komitet_najlepszych==coffee_test_labels)/64 #0.94

table(komitet_najlepszych,coffee_test_labels)

# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(labels9, labels7, labels5))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/64
...
m2_acc_k1s = sum(labels1s == coffee_test_labels)/64

acc_poszczegolnych_k1s=apply(macierz_klasyfikatorów1s==coffee_test_labels, 2, sum)/64 # 0.96
...
m2_acc_k3s= sum(labels3s == coffee_test_labels)/64

acc_poszczegolnych_k3s=apply(macierz_klasyfikatorów3s ==coffee_test_labels, 2, sum)/64
...
m2_acc_k5s = sum(labels5s == coffee_test_labels)/64

acc_poszczegolnych_k5s=apply(macierz_klasyfikatorów5s==coffee_test_labels, 2, sum)/64
...
m2_acc_k7s = sum(labels7s == coffee_test_labels)/64

acc_poszczegolnych_k7s=apply(macierz_klasyfikatorów7s==coffee_test_labels, 2, sum)/64
...
m2_acc_k9s = sum(labels9s== coffee_test_labels)/64

acc_poszczegolnych_k9s=apply(macierz_klasyfikatorów9s==coffee_test_labels, 2, sum)/64
...
write.csv(wyniki_poszczegolnych_s, '/Users/Ula/Desktop/wtum/wyniki_poszczegolnych_herring_scalone.csv')
...
table(komitet_najlepszych, coffee_test_labels)
# komitet z 1, 3, 5
etykiety_komitet = getLabels(cbind(labels3s, labels7s, labels9s))
m2_acc_komitet = sum(etykiety_komitet==coffee_test_labels)/64
